...
- 兑换码仅在活动池或限定池开池日前后 7 天，于德国时间 08:00、16:00 各检查一次，单次失败不在同一时段重试；显示数量必须跟随当前兑换码奖励位，主线/周年庆为 3 条，普通活动为 1 条。
- `confirmed` 仅表示日期是否已确认，不作为同步过滤条件；预测日期可以直接使用，后续更新时必须保留可修正能力。
- 卡池每天按德国时间 12:50、13:10 各检查一次，单次失败不在同一时段重试；追加本地不存在的新卡池，包括 `confirmed = false` 的预测卡池。
...
- 已有模块即使新增奖励阶段、刷新规则或交互方式，也只能写入当天的 `优化`，不得写入 `新增`。
- 不回写或改名历史日期下已有的日志；新进展必须在当天新增日期段中记录。
...
- 相同或相似功能必须优先调用已有逻辑，禁止未经检查自创第二套实现；只有确认无法复用时才新增代码。
...
- 除非用户特别说明，新增或调整逻辑只作用于奖励模块，不修改顶部库存、UP 数和日期区域。
- 模块弹窗统一使用兑换码弹窗的根层自适应定位；顶部 UP 弹窗保持独立逻辑。
- 圆形控件统一将外圈和内部图标分层绘制；微调内部图标只使用图标自身的 `offset`，不得改变外圈的尺寸、位置或命中区域。
- 日期必须使用真实日期、配置时区和明确结束时间，不用“一个月四周”等近似算法。
- 停服维护仅跟随活动池日期，在更新当天德国时间 11:00 自动进入新一期；更新维护保持手动刷新。
